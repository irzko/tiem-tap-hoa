...
function search(arr: ISubsubcategory[], str: string) {
  const stringNotSigned = vietnameseToAscii(str);
  const result: ISubsubcategory[] = [];
  for (let i = 0; i < arr.length; i++) {
    const string = vietnameseToAscii(arr[i].subsubcategory_name);
...
function TableHeader({
  ...
  toggle: boolean;
  setSubcategories: Dispatch<SetStateAction<ISubsubcategory[]>>;
  filteredData?: ISubsubcategory[];
  categoryId?: string;
  ...
  const { data: subcategories, isLoading } = useSWR(
    `/api/subcatgs/`,
  ...
  const onChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.value === "")
      return setSubcategories(data as ISubsubcategory[]);
    const result = search(data || [], e.target.value);
    setSubcategories(result);
  };

  const onCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    router.push(`/dashboard/sub-subcatgs/${e.target.value}`);
  };
...
function SubsubcategoryActionModal({
  showModal,
  setShowModal,
  subsubcategory,
  apiUrl,
...
function AddSubsubcategoryModal({
  ...
  const { data: categories } = useSWR(`/api/subcatgs/`, categoriesFetcher, {
    revalidateIfStale: false,
    revalidateOnFocus: false,
    revalidateOnReconnect: false,
  });
...
function DeleteSubsubcategoryModal({
  showModal,
  setShowModal,
  subsubcategory,
  apiUrl,
...
function RenameSubsubcategoryModal({
  showModal,
  setShowModal,
  subsubcategory,
  apiUrl,
  ...
  const [subcategoryName, setSubsubcategoryName] = useState<string>("");
  useEffect(() => {
    setSubsubcategoryName(subsubcategory?.subsubcategory_name || "");
  }, [subsubcategory]);
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    fetch(apiUrl, {
...
export default function Page({ params }: { params: { catgId: string } }) {
  ...
  const apiUrl = "/api/sub-subcatgs";

  const { data } = useSWR(
    `/api/subcatgs/${params.catgId}/sub-subcatgs`,
    subcategoriesFetcher,
    {
      revalidateIfStale: false,
      revalidateOnFocus: false,
