...
function CancelModal({ orderId, userId }: { orderId: string; userId: string }) {
  ...
  const handleCancel = () => {
    fetch(`/api/orders/unpaid?action=cancel`, {
      method: "PATCH",
      headers: {
  ...
  return (
    <>
      <Button onPress={onOpen}>Hủy</Button>
...
export default function Page() {
  const { data: orders, isLoading } = useSWR("/api/orders/unpaid", fetcher, {
  ...
  const renderCell = useCallback(
    ...
    [handleConfirm, session?.user.userId]
  );
