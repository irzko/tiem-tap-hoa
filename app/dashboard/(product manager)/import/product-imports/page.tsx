...
import CreateProductImportForm from "./create-product-import";
...
const getProductImports = async (): Promise<IProductImport[]> => {
  const res = await fetch(`${process.env.API_URL}/api/product-imports`);
  const data = await res.json();
  return data;
};
    ...
    <>
      <div className="max-w-screen-md mx-auto flex flex-col">
        ...
        <div className="bg-white divide-y dark:bg-gray-800 relative shadow-md sm:rounded-lg w-full text-sm text-left text-gray-500 dark:text-gray-400">
          {data?.map((item) => {
            return (
              <Card key={item.productImportId}>
              <CardHeader>{item.productImportId}</CardHeader>
              <CardBody>
                ...
                >
                  ...
                  <p>Tổng giá trị: {item.totalValue}</p>
                </div>
              </CardBody>
            </Card>
            );
